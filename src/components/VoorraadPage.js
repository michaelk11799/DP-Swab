...
import rawGroups from "../data/items";   // jouw items uit content.pdf → items.js
...
function cleanLabel(s) {
  ...
  if (
    /^(inventory|count sheet|begin date|team member|page|pulse|version|store|location)/i.test(t)
  ) {
    return "";
  }
      ...
      <div className="voorraad-box">
        ...
        <form className="voorraad-form" onSubmit={handleSubmit}>
          ...
          >
            ...
            {filteredGroups.length === 0 && (
              <option disabled>Geen items gevonden</option>
            )}
            {filteredGroups.map((group, gi) => (
              <optgroup key={gi} label={group.category}>
                {group.items.map((it, ii) => (
                  <option key={`${gi}-${ii}`} value={it}>
                    {it}
                  </option>
                ))}
              </optgroup>
        ...
        <table className="voorraad-table">
          ...
          <tbody>
            {entries.length === 0 ? (
              <tr>
                <td colSpan="8">Nog geen transacties ingevoerd.</td>
              </tr>
            ) : (
              entries.map((entry) => (
                <tr key={entry.id}>
